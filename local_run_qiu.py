...
def extor_info_worker(info_dict):
    ...
    try:
        ...
        try:
            ...
            if info_dict["origin_url"] in obj.no_needcheck_list:
                print("pass url:"+info_dict["origin_url"])
                return None
...
def parse_callback(insert_item):
    ...
    if insert_item :
        #if not DEBUG:
        mongo_table.update_one({"origin_url":insert_item['原始地址']}, { "$set":{"is_parsed":1} })
...
def mutiproces_parse(search_condition):
    ...
    for i in range(int(total_count/3000) + 1):
        ...
        #else:
        #    items = mongo_table.find(search_condition).skip(4000).limit(3000)
        process_num = 8
        queue_max_num = 10000
        print("start")
        parallel_apply(
...
if __name__ == '__main__':
    _, mongo_db = get_mongo_connect(mongo_url=MongoUrl)
    mongo_table = mongo_db['project_html']

    search_condition = {
        #'is_parsed': 0,
        #'公告类型': "公开招标公告",  # 成交公告/中标结果公告
        #'source_type': "江苏公共资源交易网",
        #"公告发布时间": {"$gt": "2020-01-01"},
        "$or":[{"公告类型":"中标公告"},{"公告类型":"中标结果公示"},{"公告类型":"中标结果公告"},{"公告类型":"成交公告"},{"公告类型":"招标公告"},{"公告类型":"招标公告/资审公告"},{"公告类型":"邀请招标公告"},{"公告类型":"公开招标公告"}]
